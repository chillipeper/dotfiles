...
def backup(dotfile):
    dotile_backup = Path(BACKUP_DIR + '/' + dotfile.name + '.' +
                         str(int(time.time())))

    if dotfile.is_file() and not dotfile.is_symlink():
        logging.info("Copying %s from %s to %s", dotfile.name, HOME_DIR,
                     BACKUP_DIR)
        shutil.copyfile(dotfile.absolute(), dotile_backup.absolute())
    else:
        logging.info(
...
def create(directory):
    ...
    if not p.exists():
        logging.info(" %s does not exist. Creating it.", p)
        p.mkdir()
    else:
...
def remove(dotfile):
    if dotfile.is_file():
        logging.info("Removing %s from %s", dotfile.name, dotfile.parent)
        dotfile.unlink()
    else:
        logging.error("%s is not a regular file, can not delete it",
                      dotfile.absolute())


def link(target, source):
    target.symlink_to(source.absolute())
    logging.info("Linking %s in home directory to %s", target.absolute(),
                 source.absolute())
